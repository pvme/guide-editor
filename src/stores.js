...
function normalizeDraft(rawDraft) {
    ...
    return createDraft({
        ...
        name: typeof draft.name === 'string' && draft.name ? draft.name : 'Untitled draft',
        content: typeof draft.content === 'string' ? draft.content : '',
        loadedGuide: draft.loadedGuide && typeof draft.loadedGuide === 'object' ? draft.loadedGuide : null,
...
function normalizeDraftState(rawState) {
    ...
    const normalizedDrafts = rawDrafts.map(normalizeDraft);
    const drafts = normalizedDrafts.length
        ? normalizedDrafts
        : [createDraft()];
...
function createDraftsStore() {
    ...
    if (typeof window !== 'undefined') {
        store.subscribe(state => {
            window.localStorage.setItem(DRAFTS_STORAGE_KEY, JSON.stringify(normalizeDraftState(state)));
        });
    }
    ...
    return {
        ...
        update: updater => store.update(state => normalizeDraftState(updater(normalizeDraftState(state)))),
        create: draftInput => {
            const draft = createDraft(draftInput);
            store.update(state => normalizeDraftState({
                ...state,
        ...
        switchTo: draftId => {
            store.update(state => state.drafts.some(draft => draft.id === draftId)
        ...
        updateActive: patch => {
            store.update(state => ({
                ...state,
                drafts: state.drafts.map(draft => draft.id === state.activeDraftId
...
export const activeDraft = derived(drafts, $drafts =>
    $drafts.drafts.find(draft => draft.id === $drafts.activeDraftId) || $drafts.drafts[0]
);
